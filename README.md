[![Mackiber Labs Comunidad](https://img.shields.io/badge/Zenodo-Mackiber%20Labs-1682b7?logo=zenodo&logoColor=white)](https://zenodo.org/communities/mackiber-labs/)
[![Licencia](https://img.shields.io/badge/Licencia-Copyright%20%28c%29%202026%20Enrique%20Aguayo-red)](LICENSE)
[![Estado](https://img.shields.io/badge/Estado-Anteproyecto-yellow)](...)
[![EN](https://img.shields.io/badge/English-version-blue.svg)](./README.en.md)

# Enrique Aguayo H. — Arquitecto de sistemas para entornos extremos


Diseño infraestructura open-source para condiciones críticas — desde ciudades densas hasta Marte.

**Filosofía:** *"Reparar el mundo. No reemplazarlo."*

---

##  Qué hago

| Área | Qué resuelvo | Proyecto insignia |
|------|--------------|-------------------|
| **Comunicaciones resilientes** | Que no caigas cuando cortan los cables | [Quantum-Flux](https://github.com/enriqueherbertag-lgtm/quantum-flux) |
| **Energía y soporte vital** | Aire limpio en ciudades y oxígeno en Marte | [ShieldAir](https://github.com/enriqueherbertag-lgtm/ShieldAir-Urban) |
| **Cuerpos artificiales** | Una plataforma física para IA o cerebro humano | [CORPUS](https://github.com/enriqueherbertag-lgtm/Corpus) |

---

##  Un ejemplo concreto

**Quantum-Flux** es una capa de comunicaciones tolerante a fallos. Divide los datos en 10 flujos a través de distintos satélites. Aunque fallen 2 o 3 enlaces, el mensaje llega completo.

[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.19358417.svg)](https://doi.org/10.5281/zenodo.19358417)

**Diseñado para:** Gobiernos, bancos e infraestructura crítica que no pueden permitirse perder conectividad.

---

##  Ecosistema

Todos los proyectos están protegidos por derechos de autor.  
**Uso no comercial** permitido con atribución.  
**Para uso comercial**, contactar a eaguayo@migst.cl

**29 proyectos en GitHub** — **24 con DOI asignado** en Zenodo.

---

##  En inglés

[Read this profile in English](./README.en.md)
